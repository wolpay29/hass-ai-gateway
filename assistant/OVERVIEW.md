...
│           └── requirements.txt
│
└── esp/                               ← ESP8266/ESP32 firmware projects
```
...
| Path | Role |
|---|---|
| [assistant/services/telegram_bot/main.py](assistant/services/telegram_bot/main.py) | Entry point; dynamically registers all handlers from `menus.yaml` |
| [assistant/services/telegram_bot/menus.yaml](assistant/services/telegram_bot/menus.yaml) | All menu/button config — edit here to change any button |
| [assistant/services/telegram_bot/bot/menu_config.py](assistant/services/telegram_bot/bot/menu_config.py) | Loads `menus.yaml`; provides helpers to the rest of the bot |
| [assistant/services/telegram_bot/bot/callbacks.py](assistant/services/telegram_bot/bot/callbacks.py) | Menu routing, `action_callback` (generic HA dispatcher), `_resolve_title` (live state in titles) |
| [assistant/services/telegram_bot/bot/menu.py](assistant/services/telegram_bot/bot/menu.py) | Reply-keyboard helpers, startup menu |
| [assistant/services/telegram_bot/bot/handlers.py](assistant/services/telegram_bot/bot/handlers.py) | Telegram voice/text handlers → `core.processor` → Markdown reply |
...
| [assistant/core/rag/embeddings.py](assistant/core/rag/embeddings.py) | Embedding HTTP client |
| [assistant/core/rag/store.py](assistant/core/rag/store.py) | sqlite-vec persistence |
| [assistant/core/llm.py](assistant/core/llm.py) | All LLM calls: parser (legacy + RAG), state formatter, smalltalk, clarification |
| [assistant/core/llm_lmstudio.py](assistant/core/llm_lmstudio.py) | LM Studio MCP fallback (Mode 2) |
| [assistant/core/ha.py](assistant/core/ha.py) | Home Assistant REST client (`get_state`, `call_service`, `get_all_states`) |
| [assistant/core/prompts.yaml](assistant/core/prompts.yaml) | All system prompts |
| [assistant/core/entities.yaml](assistant/core/entities.yaml) | Curated entity catalogue (legacy path + RAG keyword/meta overlay) |
...
### Telegram bot
```bash
cd assistant/services/telegram_bot
python main.py
```

### Voice gateway
```bash
cd assistant/services/voice_gateway
...
### Raspberry Pi client
```bash
cd assistant/devices/raspberry_pi
...
Edit `assistant/services/telegram_bot/menus.yaml` only. Restart the bot. See the `menus.yaml` section in the [Telegram bot README](assistant/services/telegram_bot/README.md) for the full syntax.
