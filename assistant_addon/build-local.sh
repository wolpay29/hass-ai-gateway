#!/usr/bin/env bash
# Stage assistant/ sources into _build/ and build the Docker image locally.
#
# Usage:
...
echo "[build-local] staging assistant sources -> $BUILD_DIR"
rm -rf "$BUILD_DIR"
mkdir -p "$BUILD_DIR/services"
cp -r "$REPO_ROOT/assistant/core"                    "$BUILD_DIR/core"
cp -r "$REPO_ROOT/assistant/services/voice_gateway"  "$BUILD_DIR/services/voice_gateway"
cp -r "$REPO_ROOT/assistant/services/notify_gateway" "$BUILD_DIR/services/notify_gateway"
cp -r "$REPO_ROOT/assistant/services/telegram_bot"   "$BUILD_DIR/services/telegram_bot"
...
IMG="local/smarthome-assistant-${ARCH}:dev"
